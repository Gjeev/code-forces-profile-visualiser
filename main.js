...
document.querySelector("handle").textContent = name.toUpperCase();
//update profile
const linkForUserInfo = "https://codeforces.com/api/user.info?handles=" + name;

async function fillProfile(url) {
  await fetch(url)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      const friends = data.result[0].friendOfCount;
      const lv = data.result[0].lastOnlineTimeSeconds;
      const photo = data.result[0].titlePhoto;
      
      document.querySelector("rating").innerHTML += rating;
      document.querySelector("mr").innerHTML += maxRating;
      document.querySelector("contribution").innerHTML += contri;
      document.querySelector("friend").innerHTML += friends;
      document.querySelector("lv").innerHTML += lv + "s";
      document.querySelector("img").setAttribute("src", photo);
      //if any of them is empty , += not available part write

    });
}
fillProfile(linkForUserInfo);

// for data that aint public
//friends
let key = "";
let secret = "";
let time = Math.round(new Date().getTime() / 1000);
let rand = "123456";
const linkForOnlineFriends =
...
async function getFriends(url1, url2) {
  await fetch(url1)
    .then((response) => {
      if (!response.ok) {
        throw new Error(`HTTP error: ${response.status}`);
    ...
    .then((data) => {
      ...
      if (data.result.length == 0) {
        document.querySelector("of").innerHTML += "none :(";
      } else {
        for (const each of data.result) {
          let frend = document.createElement("li");
          frend.appendChild(document.createTextNode(each));
          document.querySelector("of").appendChild(frend);
        }
      }
    });
  await fetch(url2)
    ...
    .then((data) => {
      ...
      if (data.result.length == 0) {
        ...
        for (const each of data.result) {
          let frend = document.createElement("li");
          frend.appendChild(document.createTextNode(each));
          document.querySelector("off").appendChild(frend);
        }
      }
    });
}
getFriends(linkForOnlineFriends, linkForAllFriends);

// contest details
const table = document.querySelector("table");
const linkForContestData = "https://codeforces.com/api/user.rating?handle=" + name;
async function getContestData(url)
{
  let x = [];
  let y = [];
  await fetch(url)
  .then((response) => {
    ...
    .then((data) => {
      ...
      for(let i = 0; i < data.result.length; i++) {
        let rowi = table.insertRow(i + 1);
        let cell1 = rowi.insertCell(0);
        let cell2 = rowi.insertCell(1);
        let cell3 = rowi.insertCell(2);
        cell1.innerHTML = data.result[i].contestName;
        cell2.innerHTML = data.result[i].oldRating;
        cell3.innerHTML = data.result[i].newRating;
        x[i] = data.result[i].contestName;
        y[i] = data.result[i].newRating;
      }
...
getContestData(linkForContestData);
